  ...
  account: {
    ...
    items: [
      ...
      },
      {
        ...
        type: "client-id",
        description:
          "Use your own Spotify Client ID to avoid rate limits. Leave empty to use shared client. Requires re-login after changing.",
      },
      {
        id: "sign-out",
        title: "Sign Out",
...
export default function Settings({
  ...
  const [clientIdValidating, setClientIdValidating] = useState(false);
  ...
  const handleSaveClientId = async () => {
    ...
    // Validate format: Spotify client IDs are 32 character hex strings
    if (!/^[a-f0-9]{32}$/i.test(trimmedId)) {
      setClientIdError("Invalid format. Must be 32 hex characters.");
      return;
    }

    // In dev mode, skip API validation (CORS blocks it)
    // On device, nocturned handles validation during auth
    const IS_DEV_MODE = import.meta.env.DEV;
    if (IS_DEV_MODE) {
      localStorage.setItem("spotifyClientId", trimmedId);
      setClientIdSaved(true);
      setTimeout(() => setClientIdSaved(false), 2000);
      return;
    }

    // Test the client ID via nocturned
    setClientIdValidating(true);
    try {
      const response = await fetch("http://127.0.0.1:5000/auth/validate-client", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ client_id: trimmedId }),
      });

      if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        if (data.error === "invalid_client" || response.status === 400) {
          setClientIdError("Invalid Client ID. Check your Spotify Developer Dashboard.");
          setClientIdValidating(false);
          return;
        }
      }

      // Success - save the client ID
      localStorage.setItem("spotifyClientId", trimmedId);
      setClientIdSaved(true);
      setTimeout(() => setClientIdSaved(false), 2000);
    } catch (err) {
      // If nocturned validation endpoint doesn't exist, just save (format was already validated)
      console.warn("Client ID validation skipped:", err.message);
      localStorage.setItem("spotifyClientId", trimmedId);
      setClientIdSaved(true);
      setTimeout(() => setClientIdSaved(false), 2000);
    } finally {
  ...
  const renderSettingItem = (item) => {
    ...
    switch (item.type) {
      ...
      case "client-id":
        ...
        return (
          <div key={item.id} className="mb-8">
            <span className="text-[32px] font-[580] text-white tracking-tight block mb-3">
              {item.title}
            </span>
            <div className="flex gap-3 mb-4">
              <button
                ...
                className={`transition-colors duration-200 rounded-[12px] px-6 py-2 focus:outline-none ${
                  !useCustomClientId
                    ? "bg-[#1db954] text-white"
                    : "bg-white/10 hover:bg-white/20 border border-white/10 text-white"
                }`}
              >
                <span className="text-[24px] font-[580] tracking-tight">
                  Use Shared
                </span>
              </button>
              <button
            ...
            {useCustomClientId && (
              <div className="flex gap-3 mb-3 items-center">
                <input
                  ...
                  className="w-full max-w-[400px] bg-white/10 border border-white/10 rounded-[12px] px-4 py-3 text-[24px] text-white placeholder-white/40 focus:outline-none focus:border-white/30"
                />
                <button
                  onClick={handleSaveClientId}
                  disabled={clientIdValidating}
                  className={`transition-colors duration-200 rounded-[12px] px-6 py-3 focus:outline-none ${
                    clientIdValidating
                ...
                >
                  <span className="text-[24px] font-[580] text-white tracking-tight">
                    {clientIdValidating ? "Validating..." : "Save"}
                  </span>
                </button>
              </div>
            )}
            {clientIdError && (
              <p className="text-[24px] font-[560] text-red-500 tracking-tight mb-2">
                {clientIdError}
              </p>
            )}
            {clientIdSaved && (
              <p className="text-[24px] font-[560] text-[#1db954] tracking-tight mb-2">
                Saved! Sign out to apply changes.
              </p>
            )}
            <p className="text-[28px] font-[560] text-white/60 max-w-[380px] tracking-tight">
              {item.description}
            </p>
            <p className="text-[20px] font-[560] text-white/40 tracking-tight mt-3">
              Shared ID: {SHARED_CLIENT_ID}
            </p>
          </div>
        );
      case "sponsors":
