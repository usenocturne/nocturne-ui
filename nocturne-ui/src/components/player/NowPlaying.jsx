...
export default function NowPlaying({
  ...
  const [debugSeekInfo, setDebugSeekInfo] = useState({ status: 'idle', position: null, lastResult: null });
  ...
  const handlePlayPause = async () => {
    if (currentPlayback?.is_playing || isPlaying) {
  ...
  // Clear skipping state when track changes (new track data arrived)
  // Also add a timeout fallback in case track data doesn't arrive
  useEffect(() => {
    if (trackId) {
      setIsSkipping(false);
    }
  ...
  // Fallback: clear skipping state after 3 seconds max
  useEffect(() => {
    if (isSkipping) {
  ...
  const handleSkipNext = useCallback(async () => {
    // Optimistic update: show skipping state and reset progress immediately
    setIsSkipping(true);
    updateProgress(0);

    const success = await skipToNext();
    // Trigger a refresh shortly after to get the new track info faster
    if (success) {
      setTimeout(() => triggerRefresh(), 300);
    } else {
      // Revert if skip failed
      setIsSkipping(false);
    }
  }, [skipToNext, triggerRefresh, updateProgress]);
  ...
  const handleWheel = useCallback(
    (e) => {
      ...
      const now = Date.now();
      if (now - lastWheelEventRef.current < 150) {
        e.preventDefault();
        return;
      }
      ...
      if (Math.abs(wheelDeltaAccumulatorRef.current) >= 2) {
        ...
        if (settings.dialSeekEnabled) {
          // Seek 10 seconds forward or backward
          const seekAmount = 10000; // 10 seconds in ms
          // Use pending position if we're in a debounce, otherwise use current progress
          const basePosition = pendingSeekPositionRef.current !== null
          ...
          if (rawNewPosition >= duration - 1000) {
          ...
          // If seeking before start, just go to 0 (don't skip to previous)
          // Only skip to previous if we're already at 0 and trying to go back
          if (rawNewPosition < 0) {
            if (basePosition <= 1000) {
              // Already near start, skip to previous
              if (seekDebounceRef.current) {
                clearTimeout(seekDebounceRef.current);
                seekDebounceRef.current = null;
              }
            ...
            // Otherwise just seek to 0
          }

          const newPosition = Math.max(0, Math.min(duration, rawNewPosition));

          // Update UI immediately (optimistic)
          updateProgress(newPosition);
          pendingSeekPositionRef.current = newPosition;

          // Update debug info
          if (isDevMode) {
            setDebugSeekInfo(prev => ({ ...prev, status: 'pending', position: newPosition }));
          }
          ...
          // Debounce: wait 1 second after last dial turn before sending API call
          seekDebounceRef.current = setTimeout(() => {
            const finalPosition = pendingSeekPositionRef.current;
            pendingSeekPositionRef.current = null;

            if (finalPosition !== null) {
              if (isDevMode) {
                setDebugSeekInfo(prev => ({ ...prev, status: 'calling API...', position: finalPosition }));
              }
              seekToPosition(finalPosition).then((success) => {
                if (isDevMode) {
                  setDebugSeekInfo(prev => ({ ...prev, status: success ? 'success' : 'failed', lastResult: success }));
                }
                if (!success) {
                  // Revert on failure - trigger refresh to get actual position
                  triggerRefresh();
                }
              });
            }
    ...
    [isProgressScrubbing, progressMs, duration, seekToPosition, updateProgress, triggerRefresh, settings.dialSeekEnabled, volume, setVolume, isDevMode, handleSkipNext, handleSkipPrevious],
  );
  ...
  const PlayPauseIcon = useMemo(() => {
    // Use isPlaying from playbackProgress for immediate UI feedback (optimistic updates)
    return isPlaying ? (
  ...
  return (
    ...
    >
        ...
        <div className="md:w-1/3 flex flex-row items-center px-12 pt-10">
          ...
          >
            <img
              ...
              className={`w-[280px] h-[280px] object-cover rounded-[12px] drop-shadow-[0_8px_5px_rgba(0,0,0,0.25)] transition-opacity duration-150 ${isSkipping ? "opacity-50" : "opacity-100"}`}
            />
          </div>
      ...
      >
        ...
        />
        {isDevMode && (
          <div className="mt-2 text-[14px] text-white/50 font-mono">
            <div>progressMs: {Math.round(progressMs)} | dialSeek: {settings.dialSeekEnabled ? 'ON' : 'OFF'}</div>
            <div>seek: {debugSeekInfo.status} | pos: {debugSeekInfo.position !== null ? Math.round(debugSeekInfo.position) : '-'} | last: {debugSeekInfo.lastResult === null ? '-' : debugSeekInfo.lastResult ? 'OK' : 'FAIL'}</div>
          </div>
        )}
      </div>
