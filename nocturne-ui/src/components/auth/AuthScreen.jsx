...
import { shouldUsePkceAuth } from "../../services/authService";
import NocturneIcon from "../common/icons/NocturneIcon";
import GradientBackground from "../common/GradientBackground";
import QRCodeDisplay from "./QRCodeDisplay";
import NetworkScreen from "./NetworkScreen";

// Get client ID info for debug display
...
const AuthScreen = ({ onAuthSuccess }) => {
  ...
  const [relayStatus, setRelayStatus] = useState(null); // null = checking, true = online, false = offline
  ...
  useEffect(() => {
    if (!AUTH_RELAY_URL) return;

    const checkRelay = async () => {
      try {
        const response = await fetch(`${AUTH_RELAY_URL}?action=test`, {
          method: 'GET',
          mode: 'cors',
        });
        const data = await response.json();
        setRelayStatus(data.status === 'ok');
      } catch (err) {
        console.error('Relay check failed:', err);
        ...
        try {
          ...
          if (!storedAccessToken || !storedRefreshToken) {
            ...
            if (authResponse?.device_code) {
              ...
              // PKCE auth flow - no device_code, tokens come via WebSocket
              setAuthInitialized(true);
              pollAuthStatus(null);
            }
          }
      ...
      <div className="relative z-10 w-full max-w-6xl px-6 grid grid-cols-2 gap-16 items-center">
        <div className="flex flex-col items-start space-y-8 ml-12">
          ...
          <div className="space-y-4">
            ...
            <p className="text-[16px] text-white/40 tracking-tight">
              {shouldUsePkceAuth() ? "PKCE" : "Device"} auth • {SPOTIFY_CLIENT_ID?.slice(0, 8) || SPOTIFY_CLIENT_ID_SHARED?.slice(0, 8)}...
            </p>
            {AUTH_RELAY_URL && (
              <p className="text-[16px] text-white/50 tracking-tight flex items-center gap-2">
                <span
                ...
                />
                Relay:  {AUTH_RELAY_URL.replace(/^https?:\/\//, '')}
              </p>
            )}
          </div>
        </div>
