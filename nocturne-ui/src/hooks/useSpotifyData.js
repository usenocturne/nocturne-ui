...
export function useSpotifyData(
  ...
  const fetchRecentlyPlayed = useCallback(async () => {
    ...
    try {
      ...
      if (response.rateLimitSkipped) return;
    ...
    try {
      ...
      const [
        ...
        networkAwareRequest(() =>
          ...
          res.ok && !res.rateLimitSkipped ? res.json() : { items: [] },
        ),
        networkAwareRequest(() =>
          fetch(
        ...
        networkAwareRequest(() =>
          ...
          .then((res) => {
            if (res.ok && !res.rateLimitSkipped) {
              return res.json();
            }
      ...
      if (topArtists.items && topArtists.items.length > 0) {
        ...
        const artistTracksPromises = artistsToFetch.map((artist) =>
          networkAwareRequest(() =>
            ...
            res.ok && !res.rateLimitSkipped ? res.json() : { tracks: [] },
          ),
        );
