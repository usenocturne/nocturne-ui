...
export const usePlaybackProgress = (
  ...
  const scheduleNextRefresh = useCallback((customInterval) => {
    // Default: 15s when playing, 60s when paused (less API calls when idle)
    const REFRESH_INTERVAL = customInterval || 15000;
  ...
  // Schedule refresh when song is about to end
  useEffect(() => {
    if (sharedState.songEndTimeoutId) {
      clearTimeout(sharedState.songEndTimeoutId);
      sharedState.songEndTimeoutId = null;
    }

    if (!isPlaying || duration <= 0) return;

    const remaining = duration - serverProgressRef.current;
    // If less than 5 seconds remaining, schedule refresh for when song ends
    if (remaining > 0 && remaining < 5000) {
      sharedState.songEndTimeoutId = setTimeout(() => {
        refreshPlaybackState();
      }, remaining + 500); // 500ms buffer after song ends
    }
  ...
  }, [isPlaying, duration, progressMs, refreshPlaybackState]);

  // Adjust polling interval based on play state
  useEffect(() => {
    if (!isPlaying) {
      // When paused, poll less frequently (60s instead of 15s)
      scheduleNextRefresh(60000);
    } else {
      // When playing, use normal 15s interval
      scheduleNextRefresh(15000);
    }
  }, [isPlaying, scheduleNextRefresh]);
  ...
  // Optimistically update playing state for immediate UI feedback
  const setOptimisticPlaying = useCallback((playing) => {
