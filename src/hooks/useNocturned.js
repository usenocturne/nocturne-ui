...
let btReconnectSettleTimer = null;
...
const setupGlobalWebSocket = async () => {
  ...
  try {
    const wsUrl = `ws://${API_BASE.replace("http://", "")}`;
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (data && data.type === "event" && data.topic === "app.ready") {
          ...
          const pendingHasLifetime =
            data.data?.hasLifetime === undefined
              ? false
        ...
        if (
          ...
          if (Object.prototype.hasOwnProperty.call(updateData, "subscribed")) {
          ...
          if (Object.prototype.hasOwnProperty.call(updateData, "hasLifetime")) {
            appHasLifetime = !!updateData.hasLifetime;
          }
          emitAppSubscribedState();
        }
...
const requestDevicesListDeduped = async (force = false) => {
  ...
  if (pendingDevicesListWsPromise) {
...
const verifyBtReconnectSettled = async (address) => {
...
const beginBtReconnectSettle = (address) => {
  if (!address) return;
