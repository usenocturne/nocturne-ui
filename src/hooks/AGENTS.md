...
17 custom hooks (9,261 lines total) forming the application's entire data/communication layer. All Spotify and daemon interaction flows through these hooks.
...
| Hook                          | Lines | Purpose                                                                                                                                                                                                                 |
| ----------------------------- | ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `useNocturned.js`             | 1,986 | **Central hub**: daemon WebSocket singleton, Bluetooth management (discovery/pairing/connect/disconnect), system updates, app-ready state, auth state, network monitoring. Exports both hooks and standalone functions. |
| `useSpotifyData.js`           | 1,375 | Library data fetching: recents, playlists, artists, liked songs, radio mixes, shows. Caching, retry logic, initial data load coordination.                                                                              |
| `useSpotifyPlayerState.js`    | 1,241 | Player state machine: now-playing events, Dealer WebSocket integration, album change detection, device type tracking.                                                                                                   |
| `useSpotifyWebSocket.js`      | 1,000 | Spotify command layer: `sendSpotifyCommand()` with UUID-correlated request/response, image fetching, device management. Wraps `useNocturned` WebSocket.                                                                 |
...
| `useImageLoader.js`           | 591   | Image preloading with canvas-based color extraction, caching, fade-in transitions.                                                                                                                                      |
| `useSpotifyPlayerControls.js` | 563   | Playback controls: play/pause/skip/seek/volume/shuffle/repeat. Volume queue processing, device switching, DJ mix support. Exports `DeviceSwitcherContext`.                                                              |
| `useGradientTransition.js`    | 480   | Album art → animated gradient background. Color extraction with canvas, smooth RGB interpolation, section-aware color schemes.                                                                                          |
| `useUpdateCheck.js`           | 321   | OTA update check/install flow via daemon API. Version comparison, download progress, reboot trigger.                                                                                                                    |
| `useLyrics.js`                | 261   | Lyrics fetch via Spotify command, time-synced highlighting, auto-scroll with manual scroll suspension.                                                                                                                  |
| `usePlaybackProgress.js`      | 258   | Client-side playback position interpolation. Tracks elapsed time between server updates, handles seek/pause/resume.                                                                                                     |
...
| Debug Bluetooth            | `useNocturned.js` → `useBluetooth()` hook, `handleBluetoothEvent()` |
