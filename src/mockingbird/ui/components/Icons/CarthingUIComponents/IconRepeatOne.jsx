...
const findClosestGlyphAvailable = (iconList, size) => {
  return iconList.find(icon => icon.size >= size) || iconList[iconList.length - 1];
};

export function IconRepeatOnce(props) {
  const { autoMirror = false, iconSize = 24, title, titleId, desc, descId, ...restProps } = props;
  ...
  const titleTag = (title, titleId) => {
    return title ? `<title ${titleId ? `id="${titleId}"` : ""}>${title}</title>` : "";
  };
  ...
  return (
    <svg
      ...
      dangerouslySetInnerHTML={{
        __html: `${titleTag(title, titleId)}${descTag(desc, descId)}${closestSize.svgContent}`
      }}
    />
  );
}

export const IconRepeatOne = IconRepeatOnce;
