...
class ShelfHeaderUiState {
  ...
  get firstLibraryCategoryId() {
    return this.yourLibraryCategories.length > 0
      ? this.yourLibraryCategories[0].parsedId
      : undefined;
  }
  ...
  get isInYourLibrary() {
    return this.yourLibraryCategories.some(
      (cat) => cat.parsedId === this.selectedCategoryId,
    );
  }
  ...
  headerItemClicked(id) {
    ...
    if (
      this.shelfStore &&
      this.shelfStore.rootStore &&
      this.shelfStore.rootStore.viewStore
    ) {
  ...
  updateToFirstItemOfCategory(categoryId) {
    ...
    if (firstItemOfCategory) {
      const itemIndex = allItems.findIndex(
        (item) => item.identifier === firstItemOfCategory.identifier,
      );
...
class ShelfSwiperUiState {
  ...
  buildContinuousShelfItems(
    ...
    const effectiveCurrentTrack =
      currentTrack || this.shelfStore.rootStore.playerStore?.state?.track;
    ...
    if (spotifyData.likedSongs) {
      ...
      items.push({
        ...
        image_id:
          spotifyData.likedSongs.images?.[0]?.url || "/images/liked-songs.webp",
        category: "playlists",
        playable: true,
      });
    }
  ...
  get INITIAL_VISIBLE_ITEMS() {
    return 5;
  }
  get INITIAL_VISIBLE_ITEMS_ON_VOICE() {
    return 9;
  }
  get NUMBER_OF_ITEMS_TO_SHOW_ON_MORE() {
    return 9;
  }
  get TIME_BEFORE_COLLAPSING_CATEGORY() {
    return 5000;
  }
  ...
  artworkClicked(item) {
    if (item.type === "CONTEXT_ITEM" && item.playable) {
      ...
      if (
        ...
        this.shelfStore.rootStore.tracklistStore.tracklistUiState.initializeTracklist(
          contextItem,
        );
    ...
    if (selectedItem) {
      if (selectedItem.type === "MORE_ITEM") {
        ...
        if (selectedItem.category === VOICE_IDENTIFIER) {
          ...
          selectedItem.category === "albums" ||
          selectedItem.category === "playlists" ||
          selectedItem.category === "artists" ||
          selectedItem.category === "podcasts" ||
