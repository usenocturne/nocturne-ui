...
class ViewStore {
  ...
  back() {
    this.resetNpvTimeout();
    if (this.viewStack.length > 1) {
      const poppedViewStackItem = this.viewStack.pop();
      this.maybeDoBackSideEffect(poppedViewStackItem);
    } else {
      this.showNpv();
    }
  }
